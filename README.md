...
- **"Supplies under contract" per-printer flag** — for vendor-managed toner (e.g. contract Canon fleets): flagged printers are excluded from the Toner Cost / Cost Per Page reports and never generate cost-entry tickets, so contract supplies don't skew your cost analytics. Replacement history stays visible on the printer page (marked *Contract*), and low-toner / predictive / critical alerts still work normally.
